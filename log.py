...
def initialize_output_dataframe(SETTINGS, PARAMS, hospital, episode):
    ...
    # General information.
    header = ["logged", "day", "location", "model name", "supply scenario", "demand scenario", "avg daily demand",
              "inventory size", "test days", "init days"]
    ...
    # RL stats.
    header += ["epsilon start", "epsilon anneal", "epsilon min", "epsilon current", "learning rate", "gamma",
               "batch size"]
    ...
    header += ["num supplied products"] + [f"num supplied {major}" for major in ABOD_names] + [f"num requests {major}"
    ...
    df.loc[:,
    "supply scenario"] = f"cau{round(SETTINGS.donor_eth_distr[0] * 100)}_afr{round(SETTINGS.donor_eth_distr[1] * 100)}_asi{round(SETTINGS.donor_eth_distr[2] * 100)}_{episode}"

    # df.loc[:,"epsilon start"] = SETTINGS.epsilon
    df.loc[:, "epsilon anneal"] = SETTINGS.end_anneal
